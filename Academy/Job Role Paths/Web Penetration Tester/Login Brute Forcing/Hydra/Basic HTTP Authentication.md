...
Start the target system via the question section at the bottom of the page. We will use the `http-get` hydra service to brute force the basic authentication target.
...
| `-P 2023-200_most_used_passwords.txt` | Password list file for brute-force attack |
| `127.0.0.1` | Target IP address (localhost) |
| `http-get /` | HTTP GET request to the root path |
| `-s 81` | Override default HTTP port and set to 81 |

### Results

Upon execution, Hydra systematically attempts each password from the wordlist against the specified resource. Once the correct password is found, you can use it to login to the website and retrieve the flag.
