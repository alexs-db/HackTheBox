...
![Profile image update prompt with upload button, placeholder image, only images allowed.](http://SERVER_IP:PORT/)
...
```bash
alexsdb@htb[/htb]$ wget https://raw.githubusercontent.com/danielmiessler/SecLists/refs/heads/master/Discovery/Web-Content/web-all-content-types.txt
alexsdb@htb[/htb]$ cat web-all-content-types.txt | grep 'image/' > image-content-types.txt
```
...
![POST request to /upload.php with file 'shell.php' disguised as image.](http://SERVER_IP:PORT/)

This time we get **File successfully uploaded**, and if we visit our file, we see that it was successfully uploaded:

![UID, GID, and groups all set to 33 (www-data).](http://SERVER_IP:PORT/profile_images/shell.php?cmd=id)
...
```bash
alexsdb@htb[/htb]$ echo "this is a text file" > text.jpg 
alexsdb@htb[/htb]$ file text.jpg 
text.jpg: ASCII text
```
...
```bash
alexsdb@htb[/htb]$ echo "GIF8" > text.jpg 
alexsdb@htb[/htb]$ file text.jpg
...
![Profile image update prompt with upload button, placeholder image, file successfully uploaded.](http://SERVER_IP:PORT/)

We can now visit our uploaded file, and we will see that we can successfully execute system commands:

![GIF8 header with UID, GID, and groups set to 33 (www-data).](http://SERVER_IP:PORT/profile_images/shell.php?cmd=id)
